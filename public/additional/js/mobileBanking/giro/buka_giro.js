$(document).ready(function () {
	$('#btnLanjut').on('click', function () {		
		...
		if(checked == true) {
			window.location.href = '/aboutGiro';
		} else {
	...
	if (form.valid() == true) {						
		if(rekeningAsal !== undefined){
			if(nominal >= 100000){			
				...
				round_error_noti('Setoral Awal Minimal Rp. 100,000');            
			}
		} else {
...
$('#btnBayar').on('click', function () {		
	...
	if (btoa(pin) == pincek) {		
		...
		var action_url = "" + base_url + "/setoranAwalGiro";            
		...
		$.ajax({
			...
			success: function (data) {
				if (data.status == 'insert_successful') {
					var param = (data.msg);
					window.location.href = '/successGiro/'+param;
				} else {
