$(document).ready(function () {
	$('#btnLanjut').on('click', function () {		
		...
		if(checked == true) {
			window.location.href = '/aboutTabungan';
		} else {
	...
	if (form.valid() == true) {						
		if(rekeningAsal !== undefined){
			if(nominal >= 50000){			
				...
				round_error_noti('Setoral Awal Minimal Rp. 50,000');            
			}
		} else {
...
$('#btnBayar').on('click', function () {		
	...
	if (btoa(pin) == pincek) {		
		...
		var action_url = "" + base_url + "/setoranAwal";            
		...
		$.ajax({
			...
			success: function (data) {
				if (data.status == 'insert_successful') {
					var param = (data.msg);
					window.location.href = '/successTabungan/'+param;
				} else {
