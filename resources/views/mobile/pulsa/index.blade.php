		...
		<form class="was-validated needs-validation" id="formSimpan">@csrf
			<div class="card shadow-sm mb-4">
				...
				<div class="card theme-bg text-white border-0 text-center">
					<div class="card-body">
						...
						<p class="text-muted mb-2">Masukkan No. Handphone</p>
					</div>
				</div>
			</div>
			...
			<ul class="nav nav-pills nav-justified tabs mb-3" id="assetstabs" role="tablist">
				<li class="nav-item" role="presentation">
					<button class="nav-link active" id="home-tab" data-bs-toggle="tab" data-bs-target="#token"
						type="button" role="tab" aria-controls="token" aria-selected="true">Pulsa</button>
				</li>
				<li class="nav-item" role="presentation">
					<button class="nav-link" id="currency-tab" data-bs-toggle="tab" data-bs-target="#tagihan"
						type="button" role="tab" aria-controls="tagihan" aria-selected="false">Paket Data</button>
				</li>
			</ul>
			<div class="tab-content" id="assetstabsContent">
				<div class="tab-pane fade show active" id="token" role="tabpanel" >					
					...
					<div class="row mb-3">	
						<input type="hidden" class="form-control" readonly name="biayaAdminPulsa" id="biayaAdminPulsa" value="{{$data['biayaAdminPulsa']}}">									
						<?php 
							$value = array(5000, 10000, 25000, 50000, 100000, 150000);
											...
											<div class="card-body">
												...
												<div class="row">
													...
													<div class="col align-self-center ps-0">
														<p class="mb-0 size-12"><span class="text-color-theme fw-medium">Pulsa</span></p>
														<p>{{number_format($value[$a])}} <small class="size-12 text-muted">30 Days remaining</small></p>
													</div>
													<div class="col-auto">
					...
					<button id="btnLanjutPulsa" type="button" class="btn btn-lg btn-default btn-action w-100 mb-4 shadow">
                        Lanjutkan
                    </button>
				</div>
				<div class="tab-pane fade" id="tagihan" role="tabpanel" aria-labelledby="currency-tab">					
					<div class="row mb-3">						
						<input type="hidden" class="form-control" readonly name="biayaAdminPaketData" id="biayaAdminPaketData" value="{{$data['biayaAdminPaketData']}}">				
						<?php 
							$value = array(5000, 10000, 20000, 30000, 400000, 50000);
							$paketData = array("Internet 2 Gb", "Internet 6 Gb", "Internet 8 Gb", "Internet 13 Gb", "Internet 20 Gb", "Internet 35 Gb");
							for($a=0; $a<count($value); $a++){
						?>
							<div class="col-12 col-md-6 col-lg-6">
								<div class="swiper-container cardswiper">
									<div class="swiper-slide">
										<div class="card shadow-sm mb-3">
											<div class="card-body">
												<div class="form-check position-absolute end-0 bottom-0 m-1">											
													<input value="{{$value[$a].'|'.$paketData[$a]}}" class="form-check-input rounded-circle" name="nominal_data" type="radio">
													<label for="rekening" class="form-check-label"></label>
												</div>
												<div class="row">
													<div class="col-auto">
													...
													<div class="col align-self-center ps-0">
														<p class="mb-0 size-12"><span class="text-color-theme fw-medium">Paket Data</span></p>
														<p>{{number_format($value[$a])}} <small class="size-12 text-muted">{{$paketData[$a]}}</small></p>
													</div>
													<div class="col-auto">
														<button class="btn btn-default btn-44 shadow-sm">
															<i class="bi bi-arrow-up-right-circle"></i>
					...
					<button id="btnLanjutData" type="button" class="btn btn-lg btn-default btn-action w-100 mb-4 shadow">
                        Lanjutkan
                    </button>					
				</div>
			</div>
						...
						<table class="styled-table">
							...
							<tr height="40px">								
								...
								<td style="text-align:right; padding: 10px;">
									...
									<input type="hidden" class="form-control" readonly name="inetInput" id="inetInput"> 
								</td>
							</tr>                                
							<tr height="40px">
...
@push('scripts')	
	<script src="{{ asset('additional/js/mobileBanking/pulsa/pulsa.js?v=1.00') }}"></script>	
@endpush  